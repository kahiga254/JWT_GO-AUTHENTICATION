...
func (app *Application) AddToCart() gin.HandlerFunc{
	...
	err = database.AddProductToCart(ctx, app.userCollection, app.prodCollection, userQueryID, productID)
	if err != nil {
...
func (app *Application) RemoveItem() gin.HandlerFunc{
		return func(c *gin.Context){
			...
			err = database.RemoveCartItem(ctx, app.userCollection, app.prodCollection, userQueryID, productID) 
			if err != nil {
				c.IndentedJSON(http.StatusInternalServerError, err)
				return
			}
...
func (app *Application) InstantBuy() gin.HandlerFunc{
	...
	return func(c *gin.Context){
		productQueryID := c.Query("id")
			...
			var ctx, cancel = context.WithTimeout(context.Background(), 5*time.Second)
			defer cancel()	
			err = database.InstantBuyer(ctx, app.userCollection, app.prodCollection, userQueryID, productID)
			if err != nil {
				c.IndentedJSON(http.StatusInternalServerError, err)
				return
			}
